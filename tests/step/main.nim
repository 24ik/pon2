...
proc main*() =
  ...
  block:
    ...
    block:
      ...
      check step.optPlacement == Opt[Placement].ok Down3
  ...
  block:
    ...
    check $step == "bp|43"
    check "bp|43".parseStep == Result[Step, string].ok step

    check step.toUriQuery(Pon2) == Result[string, string].ok "bp43"
    check step.toUriQuery(Ishikawa) == Result[string, string].ok "QG"
    check step.toUriQuery(Ips) == Result[string, string].ok "QG"

    check "bp43".parseStep(Pon2) == Result[Step, string].ok step
    check "QG".parseStep(Ishikawa) == Result[Step, string].ok step
    check "QG".parseStep(Ips) == Result[Step, string].ok step
  ...
  block:
    block:
      ...
      check "(2,3,3,2,2,3)".parseStep == Result[Step, string].ok step

      check step.toUriQuery(Pon2) == Result[string, string].ok "o2_3_3_2_2_3o"
      check step.toUriQuery(Ishikawa) == Result[string, string].ok "yp"
      check step.toUriQuery(Ips) == Result[string, string].ok "yp"

      check "o2_3_3_2_2_3o".parseStep(Pon2) == Result[Step, string].ok step
      check "yp".parseStep(Ishikawa) == Result[Step, string].ok step
      check "yp".parseStep(Ips) == Result[Step, string].ok step

    block:
      let step = Step.init([Col0: 0, 0, 0, -1, 0, 0])
      check step.toUriQuery(Pon2) == Result[string, string].ok "o0_0_0_-1_0_0o"
  ...
  block:
    ...
    check $steps == "rg|\n(1,0,0,0,0,1)\nyy|3N"
    check "rg|\n(1,0,0,0,0,1)\nyy|3N".parseSteps == Result[Steps, string].ok steps

    check steps.toUriQuery(Pon2) == Result[string, string].ok "rgo1_0_0_0_0_1oyy3N"
    check steps.toUriQuery(Ishikawa) == Result[string, string].ok "c1axG4"
    check steps.toUriQuery(Ips) == Result[string, string].ok "c1axG4"

    check "rgo1_0_0_0_0_1oyy3N".parseSteps(Pon2) == Result[Steps, string].ok steps
    check "c1axG4".parseSteps(Ishikawa) == Result[Steps, string].ok steps
    check "c1axG4".parseSteps(Ips) == Result[Steps, string].ok steps
