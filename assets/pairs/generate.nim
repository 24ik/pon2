...
when isMainModule:
  ...
  block:
    ...
    let
      dstPath =
        currentSourcePath().parentDir.parentDir.parentDir / "src" / "pon2" / "private" /
        "app" / "marathon" / "pairs.nim"
      pairsTxt = lines.join "\n"
    dstPath.writeFile &"""
# This file is generated by the program, so do *NOT* edit it manually.
# Note that this module has a very memory-consuming constant, so it should
# *NOT* be imported if not needed.
const MarathonPairsTextRaw* =
  when defined(nimdoc): "" # HACK: nimdoc cannot handle very long strings.
  else: {TripleQuote}
