...
type
  Bit64BinField* = object ## Binary field with 64bit operations.
    left: uint64 # only use higher 16*3 bits
    right: uint64 # only use higher 16*3 bits
...
func init(T: type Bit64BinField, left: uint64, right: uint64): T {.inline.} =
...
func `+`*(f1, f2: Bit64BinField): Bit64BinField {.inline.} =
  Bit64BinField.init(f1.left or f2.left, f1.right or f2.right)

func `-`*(f1, f2: Bit64BinField): Bit64BinField {.inline.} =
  Bit64BinField.init(f1.left.clearMasked f2.left, f1.right.clearMasked f2.right)

func `*`*(f1, f2: Bit64BinField): Bit64BinField {.inline.} =
  Bit64BinField.init(f1.left and f2.left, f1.right and f2.right)

func `*`(self: Bit64BinField, val: uint64): Bit64BinField {.inline.} =
  Bit64BinField.init(self.left and val, self.right and val)

func `xor`*(f1, f2: Bit64BinField): Bit64BinField {.inline.} =
  Bit64BinField.init(f1.left xor f2.left, f1.right xor f2.right)
...
func sum*(f1, f2, f3: Bit64BinField): Bit64BinField {.inline.} =
  Bit64BinField.init(
    bitor2(f1.left, f2.left, f3.left), bitor2(f1.right, f2.right, f3.right)
  )
...
func prod*(f1, f2, f3: Bit64BinField): Bit64BinField {.inline.} =
  Bit64BinField.init(
    bitand2(f1.left, f2.left, f3.left), bitand2(f1.right, f2.right, f3.right)
  )
...
func colMaskLeft(colOrd: int): uint64 {.inline.} =
  ## Returns the mask for the left with only the column bits one.
  0xffff_0000_0000_0000'u64 shr (colOrd shl 4)

func colMaskRight(colOrd: int): uint64 {.inline.} =
  ## Returns the mask for the right with only the column bits one.
  0x0000_0000_ffff_0000'u64 shl ((Col.high.ord - colOrd) shl 4)
...
func kept*(self: Bit64BinField, col: Col): Bit64BinField {.inline.} =
  ...
  Bit64BinField.init(
    self.left and col.ord.colMaskLeft, self.right and col.ord.colMaskRight
  )
...
func keepValid(self: var uint64) {.inline.} =
  ## Keeps only the valid area.
...
func cntOne*(self: Bit64BinField): int {.inline.} =
  ## Returns the number of one bits.
  self.left.countOnes + self.right.countOnes
...
func shiftRightRaw*(self: var Bit64BinField) {.inline.} =
  ...
  self.right.assign (self.left shl 32) or (self.right shr 16)

func shiftLeftRaw*(self: var Bit64BinField) {.inline.} =
  ## Shifts the binary field rightward.
  self.left.assign (self.left shl 16) or (self.right shr 32)
...
func flipV*(self: var Bit64BinField) {.inline.} =
  ## Flips the binary field vertically.
  self.left.assign (self.left.reverseBits shl 31).flipped
  self.right.assign (self.right.reverseBits shl 31).flipped

func flipH*(self: var Bit64BinField) {.inline.} =
...
func flippedV*(self: Bit64BinField): Bit64BinField {.inline.} =
  ## Returns the binary field flipped vertically.
  self.dup flipV

func flippedH*(self: Bit64BinField): Bit64BinField {.inline.} =
...
func `[]`*(self: Bit64BinField, row: static Row, col: static Col): bool {.inline.} =
  when col in {Col0, Col1, Col2}:
    ...
    self.right.getBitBE static(idxFromMsb(row, col.pred 3))

func `[]`*(self: Bit64BinField, row: static Row, col: Col): bool {.inline.} =
  case col
  of Col0, Col1, Col2:
    self.left.getBitBE idxFromMsb(row, col)
  of Col3, Col4, Col5:
    self.right.getBitBE idxFromMsb(row, col.pred 3)
...
func `[]=`*(self: var Bit64BinField, row: Row, col: Col, val: bool) {.inline.} =
  ...
  of Col3, Col4, Col5:
    self.right.changeBitBE idxFromMsb(row, col.pred 3), val
...
func insert(
    self: var uint64, col0OrCol1OrCol2: Col, row: Row, val: bool, rule: static Rule
) {.inline.} =
  ...
  ## inserted.
  let
    colShift = col0OrCol1OrCol2.ord shl 4
...
func insert*(
  ...
  of Col3, Col4, Col5:
    self.right.insert col.pred 3, row, val, rule

func delete(
    self: var uint64, col0OrCol1OrCol2: Col, row: Row, rule: static Rule
) {.inline.} =
...
func delete*(
  ...
  of Col3, Col4, Col5:
    self.right.delete col.pred 3, row, rule
...
func colVal(self: Bit64BinField, col: static Col): uint64 {.inline.} =
  ...
  when col == Col0:
    self.left.bitsliced 48 ..< 64
  elif col == Col1:
    self.left.bitsliced 32 ..< 48
  elif col == Col2:
    self.left.bitsliced 16 ..< 32
  elif col == Col3:
    self.right.bitsliced 48 ..< 64
  elif col == Col4:
    self.right.bitsliced 32 ..< 48
  else:
...
func toDropMask*(existField: Bit64BinField): Bit64DropMask {.inline.} =
  ...
  var dropMask {.noinit.}: Bit64DropMask
  staticFor(col, Col):
