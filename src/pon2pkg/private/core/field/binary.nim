...
import ../../[intrinsic]
import ../../../corepkg/[fieldtype, position, rule]
...
func isDead*(self; rule: Rule): bool {.inline.} =
  ...
  case rule
  of Tsu: bool self.exist(1, 2)
  of Water: not self.row(WaterRow.low.pred).isZero
...
func validDoublePositions*(self): set[Position] {.inline.} =
  ...
  DoublePositions - self.invalidPositions
...
func flipV*(mSelf) {.inline.} = mSelf = mSelf.flippedV
  # Flips the binary field vertically.

func flipH*(mSelf) {.inline.} = mSelf = mSelf.flippedH
...
func disappeared(connection: Connection): BinaryField {.inline.} =
  ...
  let
    connect4Up =
      connection.connect3IL * connection.connect3IL.shiftedUpWithoutTrim
    connect4Down =
      connection.connect3IL * connection.connect3IL.shiftedDownWithoutTrim
    connect4Right =
      connection.connect3IL * connection.connect3IL.shiftedRightWithoutTrim
    connect4Left =
      connection.connect3IL * connection.connect3IL.shiftedLeftWithoutTrim

  result = connection.visible * (
    sum(connection.connect4T, connect4Up, connect4Down, connect4Right,
        connect4Left)).expanded

func disappeared*(self): BinaryField {.inline.} = self.connections.disappeared
  ## Returns the binary field where four or more cells are connected.
...
func connect3*(self): BinaryField {.inline.} =
  ...
  result =
    connection.connect3IL.expanded * connection.visible - connection.disappeared
...
func connect3L*(self: BinaryField): BinaryField {.inline.} =
  ...
  ## This function ignores ghost puyos.
  let connection = self.connections
  result = connection.connect3IL.expanded * connection.visible - sum(
