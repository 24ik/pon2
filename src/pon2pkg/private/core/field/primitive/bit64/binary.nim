...
type
  ...
  DropMask* = array[Column, when UseBmi2: uint64 else: PextMask[uint64]]
...
func popcnt*(self): int {.inline.} = self.left.popcount + self.right.popcount
  ## Population count.
...
func airTrimmed*(self): BinaryField {.inline.} =
  ...
  self * BinaryField(left: 0x0000_3E00_3E00_3E00'u64,
                     right: 0x3E00_3E00_3E00_0000'u64)
...
func cellMasks(row, col): tuple[left: uint64; right: uint64] {.inline.} =
  ...
  result.right =
    (0x2000_0000_0000_0000'u64 shr (16 * (col - 3) + row)) and rightMask
...
func belowMasks(row, col): tuple[left: uint64; right: uint64] {.inline.} =
  ...
  let (leftMask, rightMask) = col.leftRightMasks
  result.left = bitand(
    uint64.high.masked 16 * (2 - col) .. 16 * (2 - col) + Row.high - row + 1,
    leftMask)
  result.right = bitand(
...
func toDropMask*(existField: BinaryField): DropMask {.inline.} =
  ...
  result[Column.low] =
    when UseBmi2: 0 else: 0'u64.toPextMask # dummy to remove warning
  for col in Column.low..Column.high:
    result[col] = when UseBmi2: arr[col] else: arr[col].toPextMask
...
func drop*(mSelf; mask: DropMask) {.inline.} =
  ...
  mSelf.left = bitor(
    arr[0].pext(mask[0]) shl 33, arr[1].pext(mask[1]) shl 17,
    arr[2].pext(mask[2]) shl 1)
  mSelf.right = bitor(
    arr[3].pext(mask[3]) shl 49, arr[4].pext(mask[4]) shl 33,
...
func waterDrop*(waterDropExistField, dropField,
  ...
  return dropField * BinaryField(left: left, right: right) +
