...
proc getParallelCount(id: kstring): int {.importjs:
  &"document.getElementById('{ParallelCountSelectIdPrefix}' + (#)).selectedIndex + 1".}
  ## Returns the parallel count.

proc initEditorSettingsNode*(editorPermuter: var EditorPermuter, id = ""): VNode
                            {.inline.} =
...
proc getSettings*(id: string, moveCount: Positive): tuple[
    parallelCount: int, fixMoves: seq[Positive], allowDouble: bool,
